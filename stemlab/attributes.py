...
class SelectRegister(BaseRegister, SelectProperty):
    ...
    def get_value(self, obj):
        ...
        if raw_value != expected_value:
            ...
            # Changed code to prioritize FGPA register settings over module
            # settings to prevent pc reboots from changing hardware_module
            # settings on the FPGA
            options = SelectProperty.options(self)
            option = list(options.keys())[list(options.values()).index(raw_value)]
            self.set_value(obj, option)
            value = SelectProperty.get_value(self, obj)
        return value
